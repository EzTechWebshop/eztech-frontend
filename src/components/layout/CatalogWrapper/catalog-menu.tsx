    ...
    return (
        <div className="flex flex-col w-full md:flex-1 gap-2">
            ...
            <div className="flex justify-between">
                <form onSubmit={(e) => handleSearch(e)} className="flex items-center gap-2">
                    <Input
                        ...
                        value={searchValue}
                    />
                    <Button disabled={loading} size={"badge"} variant={"outline"} type="submit">
                        <IoSearch />
                    </Button>
                </form>
                <Button disabled={loading} buttonTip="Refresh" variant={"ghost"} size={"badge"} onClick={() => router.push("/catalog")}>
            ...
            <div className="flex flex-col gap-2 sm:gap-0 sm:flex-row sm:flex-1 sm:justify-between sm:items-center">
                ...
                <IncludeSoldOutSwitch loading={loading} setLoading={setLoading} includeSoldOut={includeSoldOut} />
            </div>
        </div>
    );
}
...
const IncludeSoldOutSwitch = ({ ...props }: IncludeSoldOutSwitchProps) => {
    ...
    return (
        <div className="flex items-center gap-0 w-fit sm:gap-2">
            ...
            <Switch id="include-sold-out" checked={includeSoldOut ?? false} disabled={loading} onClick={handleSoldOutSwitch} />
        </div>
    );
};
