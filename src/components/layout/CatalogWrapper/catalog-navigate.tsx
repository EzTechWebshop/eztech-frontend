...
export default function CatalogNavigate({ ...props }: CatalogNavigateProps) {
    ...
    return (
        <div className="flex items-center select-none gap-4">
            <Button disabled={page <= 1 || loading} onClick={() => changePage(-1)} variant={"outline"}>
                <IoArrowBackCircleOutline size={24} />
            </Button>
            <p className="whitespace-nowrap">
                {page} / {totalPages}
            </p>
            <Button disabled={page >= totalPages || loading} onClick={() => changePage(1)} variant={"outline"}>
                <IoArrowForwardCircleOutline size={24} />
            </Button>
            <div className="flex">
                {pageSizes.map((size) => (
                    <NavigatePageSizeButton key={size} handlePageSize={handlePageSize} size={size} />
                ))}
            </div>
        </div>
    );
