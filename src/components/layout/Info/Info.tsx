        ...
        <div className="flex flex-1 gap-4 max-w-7xl">
            <div className="flex flex-col w-full">
                ...
                <div className="flex flex-col mt-4 space-y-8">
                    {infoList.length == 0 && <Heading size={"7"}>No info found</Heading>}
                    {infoList.map((item) => (
    ...
    return (
        <div className="flex space-x-4 py-4 border-b-2 justify-between px-8">
                    ...
                    <DropdownMenuContent>
                        {typeList.map((item) => (
                            <Link href={`/about-us/${item}`} key={item}>
                                <DropdownMenuItem className={cn("", type == item && "bg-slate-200")}>
                                    <Text>{WebsiteInfoFieldTopicNames[item]}</Text>
                                </DropdownMenuItem>
                            </Link>
                        ))}
                    </DropdownMenuContent>
            ...
            <div className="hidden md:flex flex-wrap items-center overflow-hidden space-x-4">
                {typeList.map((item) => (
                    <Link href={`/about-us/${item}`} key={item}>
                        <Button className={cn("", type == item && "bg-slate-200")} variant={"outline"} size={"xs"}>
                            <Text size={"1"}>{WebsiteInfoFieldTopicNames[item].toString()}</Text>
                        </Button>
                    </Link>
                ))}
            </div>
