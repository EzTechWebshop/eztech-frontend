...
export default function EditProductCategoriesModal({ ...props }: EditProductCategoriesModalProps) {
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button onClick={handleClick} buttonTip="Edit categories associated with the product" variant="iconCircle" size="icon">
                    <IoPencilSharp size={20} />
                </Button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>Categories</DialogHeader>
                {message && <div className={cn("", added ? "text-green-500" : "text-red-500")}>{message}</div>}
