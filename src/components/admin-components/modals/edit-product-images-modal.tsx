...
export default function EditProductImagesModal({
    ...props
}: EditProductImagesDialogProps) {
    ...
    const handleDeleteImage = async (image: any) => {
        if (
            !ConfirmationWindow('Are you sure you want to delete this image?')
        ) {
            return
        }
    ...
    const handleAddImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const result = await AddProductImage(product.id, formData).catch(
            (err) => {
                                ...
                                <div className="flex flex-1 justify-between mx-4">
                                    <div className="flex items-center space-x-2">
                                        <Switch
                                            ...
                                            onClick={() =>
                                                handleChoosePrimaryImage(image)
                                            }
                                        />
                                        <Label htmlFor="include-sold-out">
                                            Is Primary
                                        </Label>
                                    </div>
                                    <Button
