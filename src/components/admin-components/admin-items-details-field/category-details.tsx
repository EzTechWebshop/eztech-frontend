            ...
            <DetailsBox label="Products" className="w-[75%]">
                {products.length > 0 ? (
                    <ScrollArea className="max-h-[50vh] overflow-auto px-4">
                        <div className={`grid gap-2 grid-cols-2 ${true ? "" : ""}`}>
                            {products.map((item) => (
                                <ProductDetailsModal key={category.id} product={item} />
                            ))}
                        </div>
                    </ScrollArea>
                ) : (
...
function ProductDetailsModal({ ...props }: ProductDetailsModalProps) {
    ...
    return (
        <div
            ...
            <DetailText label="Created At" text={new Date(product.createdAt).toLocaleDateString("en-GB")} />
            <DetailText label="Status" text={product.isDeleted ? "Inactive" : "Active"} />
        </div>
    );
}
