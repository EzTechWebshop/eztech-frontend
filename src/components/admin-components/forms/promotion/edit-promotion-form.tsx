...
export function EditPromotionForm({ ...props }: EditPromotionModalProps) {
    ...
    const onSubmit = async (data: z.infer<typeof editPromotionFormSchema>) => {
        if (
            !ConfirmationWindow('Are you sure you want to edit this promotion?')
        ) {
            return
        }
        ...
        const result = await UpdatePromotion(promotion.id, request).catch(
            (err) => {
