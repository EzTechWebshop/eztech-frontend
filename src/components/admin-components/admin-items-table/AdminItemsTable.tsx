    ...
    return (
        <ScrollArea className="max-h-[calc(100vh-64px-48px)] overflow-auto">
            <Table className="border-2">
                ...
                <TableBody>
                    {props.type === 'products' && (
                        <ProductTableBody items={props.items} />
                    )}
                    {props.type === 'orders' && (
                        <OrderTableBody items={props.items} />
                    )}
                    {props.type === 'categories' && (
                        <CategoryTableBody items={props.items} />
                    )}
                    {props.type === 'promotions' && (
                        <PromotionTableBody items={props.items} />
                    )}
                </TableBody>
            </Table>
        </ScrollArea>
...
const TableHeadButton = ({ ...props }: TableHeadButtonProps) => {
    ...
    const changeSort = () => {
        ...
        if (currentQuery[0] !== label.toLowerCase()) {
            ...
            result = HandleQueryChange(
                'sort',
                `${label.toLowerCase()}_${newSort}`
            )
        }
        router.push('?' + result)
    }
