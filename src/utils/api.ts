...
import { AddProductRequest, UpdateProductRequest } from "@/types/admin-types/admin-product-types";
import { CatalogPromotions } from "@/types/public-types";
import { AddPromotionRequest, UpdatePromotionRequest } from "@/types/admin-types/admin-promotion-types";
...
const management = {
    getProductManagement: async (searchQuery?: string) => {
        ...
        const res = await api.get(`/api/admin/management/get-products${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    getCategoryManagement: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/management/get-categories${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    getOrderManagement: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/management/get-orders${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    ...
    getWebsiteInfoManagement: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/management/get-website-info${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    getPromotionManagement: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/management/get-promotions${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
};
const product = {
    getProducts: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/get-products${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    ...
    removeProduct: async (productId: number) => {
        const api = await protectedApi();
        const res = await api.delete(`/api/admin/product/remove-product/${productId}`);
        return res;
    },
    restoreProduct: async (productId: number) => {
        const api = await protectedApi();
        const res = await api.post(`/api/admin/product/restore-product/${productId}`);
        return res;
    },
    ...
    addCategoryToProduct: async (productId: number, categoryId: number) => {
        const api = await protectedApi();
        const res = await api.post(`/api/admin/product/${productId}/add-category/${categoryId}`);
        return res;
    },
    removeCategoryFromProduct: async (productId: number, categoryId: number) => {
        const api = await protectedApi();
        const res = await api.post(`/api/admin/product/${productId}/remove-category/${categoryId}`);
        return res;
    },
    // Image Management
    async addProductImage(productId: number, image: any) {
        const api = await protectedApi();
        const res = await api.post(`/api/admin/product/${productId}/add-image`, image, {
            headers: {
    ...
    async deleteProductImage(productId: number, imageId: number) {
        const api = await protectedApi();
        const res = await api.delete(`/api/admin/product/${productId}/remove-image/${imageId}`);
        return res.data;
    },
    async setProductThumbnail(productId: number, imageId: number) {
        const api = await protectedApi();
        const res = await api.post(`/api/admin/product/${productId}/set-thumbnail/${imageId}`);
        return res.data;
    },
};
const category = {
    getCategories: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/category${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
...
const order = {
    getOrders: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/get-orders${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
...
const websiteinfo = {
    ...
    editWebsiteInfoText: async (id: number, request: UpdateWebsiteInfoTextRequest) => {
        const api = await protectedApi();
        const res = await api.patch(`/api/admin/websiteinfo/update-info-text/${id}`, request);
        return res;
    },
    deleteWebsiteInfoText: async (id: number) => {
        const api = await protectedApi();
        const res = await api.delete(`/api/admin/websiteinfo/delete-info-text/${id}`);
        return res;
    },
};
const dashboard = {
...
const promotion = {
    getPromotions: async (searchQuery?: string) => {
        const api = await protectedApi();
        const res = await api.get(`/api/admin/get-promotions${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    ...
    addPromotionImage: async (productId: number, image: any) => {
        const api = await protectedApi();
        const res = await api.post(`/api/admin/promotion/${productId}/add-image`, image, {
            headers: {
                "Content-Type": "multipart/form-data",
            },
        });
...
const catalog = {
    getProducts: async (searchQuery?: string) => {
        const res = await unprotectedApi.get(`/api/catalog/get-products${searchQuery ? `${searchQuery}` : ""}`);
        return res;
    },
    getProductById: async (productId: number) => {
        const res = await unprotectedApi.get(`/api/catalog/get-product-by-id/${productId}`);
        return res;
    },
    getFaqs: async () => {
    ...
    getWebsiteInfo: async () => {
        const res = await unprotectedApi.get<WebsiteInfo>(`/api/catalog/get-website-info`);
        return res;
    },
    getPromotions: async () => {
        const res = await unprotectedApi.get<CatalogPromotions>(`/api/catalog/get-promotions`);
        return res;
    }
};
